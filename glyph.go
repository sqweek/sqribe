...
type SvgPath struct {
	...
	bl, bt, br, bb float64 // borders for svg rendering as percentage [0, 1]
	path []svg.Segment
	bounds svg.Bounds
	border svg.Bounds
}
...
func (p SvgPath) ScaleToHeight(height int) Glyph {
	...
	r.Min.X -= int(math.Ceil(float64(width) * p.bl))
	r.Min.Y -= int(math.Ceil(float64(height) * p.bt))
	r.Max.X += int(math.Ceil(float64(width) * p.br))
	r.Max.Y += int(math.Ceil(float64(height) * p.bb))
	return svg.PathMask(p.path, p.border, r)
}

func mustMkSvgPath(x, y, bl, bt, br, bb float64, path string) (p SvgPath) {
	var err error
	p.x, p.y = x, y
	p.path, err = svg.ParsePath(path)
	p.bounds = svg.PathBounds(p.path)
	p.border = p.bounds.Border(bl, bt, br, bb)
	p.bl, p.br = bl / p.bounds.Dx(), br / p.bounds.Dx()
	p.bt, p.bb = bt / p.bounds.Dy(), bb / p.bounds.Dy()
	if err != nil {
		panic(err)
	}
	return
